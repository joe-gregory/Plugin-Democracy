...
const communitySchema = new Schema(
				...
				type: new mongoose.Schema(
					{
						...
						status: {
							type: String,
							enum: ["proposal", "active", "passed", "inactive"],
							required: true,
							default: "proposal",
						}, //status of record
	...
	//Community options
	{
		...
		methods: {
			//all methods take in an object where all the inputs reside with the same structure as the schema
			...
			generateUniqueIdentifier: function () {
				...
				while (!success) {
					...
					if (
						...
						!this.reservedIdentifiers.includes(identifier)
					)
						success = true;
				}
				return identifier;
			...
			isCitizenMember: function (citizen_id) {
				...
				return this.citizens.some((citizen) =>
					citizen.equals(citizen_id)
				);
			},

			isCitizenResident: function (citizen_id) {
				return this.residents.some((resident) =>
					resident.equals(citizen_id)
				);
			},
			...
			getHome: function (input) {
				let home = this.homes.find(
					(home) =>
						home._id === input.home._id ||
						home.id === input.home.id ||
						home.number === input.home.number
				);
				if (home === undefined)
					throw new Error("No home found with provided information");
				return home;
			},
			...
			getCitizen: async function (input) {
				...
				if (!citizen)
					throw new Error("No citizen found with given _id");
				return citizen;
			},
			...
			homesOwned: function (citizenId) {
				...
				for (const home of this.homes) {
					if (home.owner && home.owner.equals(citizenId))
						homes.push(home);
				}
				return homes;
			},
			...
			majorityVotes: function (record) {
				...
				if (this.votingUnit === "community.citizens") {
					let amountCitizenVotes = record.votes.reduce(
						(total, vote) => {
					...
					result =
						amountCitizenVotes > this.citizens / 2 ? true : false;
					return result;
					...
					for (const vote of record.votes) {
						...
						if (this.isCitizenOwner(vote.citizen)) {
							let amount_homes_owned = this.homesOwned(
								vote.citizen
							).length;
							if (vote.vote === "plug")
								votesInFavor += amount_homes_owned;
							else if (vote.vote === "unplug")
								votesAgainst += amount_homes_owned;
						}
					}
			...
			law2Proposal: async function (record) {
				if (
					record.proposalLimit < 1 ||
					record.previousProposalLimit < 1
				) {
					throw new Error(
						"Proposed proposal limit below 1 or wrong type"
					);
				}
				...
				law2.body =
					...
					this.proposalLimit +
					" días a partir de la fecha de su publicación. Los ciudadanos pueden " +
			...
			addPost: async function (input) {
				...
				if (input.record) {
					...
					if (input.record.author) {
						input.record.author = await this.getCitizen({
							citizen: { _id: input.record.author },
						});
					}
				}
				...
				if (input.record) {
					...
					switch (input.record.type) {
						...
						default:
							throw new Error(
								'No input.record.type matched in "vote" case for addPost'
							);
					}
				}
				...
				switch (input.post.type) {
					...
					default:
						throw new Error(
							"No post type determined in addPost method"
						);
				}
			...
			updateRecord: async function (record) {
				...
				if (this.reservedIdentifiers.includes(record.identifier)) {
					...
					result.message =
						"updateRecord will not update records with reserved identifiers";
					return result;
				}

				let within_proposal_time_limit =
					Date.now() - record.createdAt.getTime() <=
					this.proposalLimitMilliseconds
						? true
						: false; //if proposal if still within the time limit that you can vote on a proposal (community.proposalLimit)
				let majorityVotes = this.majorityVotes(record); //true if it has majority votes
				let within_record_expiration;
				if (record.expirationDate)
					within_record_expiration =
						record.expirationDate.getTime() > Date.now()
							? true
							: false;
				//when law expires
				...
				if (record.effectiveDate)
					after_record_effective_date =
						record.effectiveDate.getTime() <= Date.now()
							? true
							: false;
				...
				//Determination
				if (within_record_expiration === false)
			...
			updateAllRecords: async function () {
				this.records.forEach(
					async (record) => await this.updateRecord(record)
				);
			},
			...
			reorderRecords: async function (type = "law") {
				...
				let newestRecord = this.records.sort(
					(a, b) => b.statusUpdateDate - a.statusUpdateDate
				)[0];

				if (newestRecord.type !== type) return;

				if (newestRecord.lawCategory && type === "law") {
					let categoryOrder = this.records.filter(
						(r) =>
							r.lawCategory === newestRecord.lawCategory &&
							r.status === "active"
					);
					categoryOrder = categoryOrder.filter(
						(r) => r.identifier !== newestRecord.identifier
					);
					categoryOrder.sort(
					...
					if (newestRecord.status === "active") {
						...
						if (categoryOrder.length === 0) {
							...
							biggestCategoryNumber =
								categoryOrder[categoryOrder.length - 1]
									.lawCategoryNumber;
						}
						if (!newestRecord.lawCategoryNumber) {
							newestRecord.lawCategoryNumber =
								biggestCategoryNumber + 1;
						...
						for (let i = 0; i < categoryOrder.length; i++) {
							if (
								...
								(categoryOrder[i + 1] === undefined ||
									newestRecord.lawCategoryNumber <=
										categoryOrder[i + 1].lawCategoryNumber)
							) {
						...
						newestRecord.status === "inactive" ||
						newestRecord.status === "passed"
					) {
						for (let i = 0; i < categoryOrder.length; i++) {
							categoryOrder[i].lawCategoryNumber = i + 1;
						}
					...
					let numberOrder = this.records.filter(
						(r) =>
							r.status === "active" &&
							r.type === type &&
							!r.lawCategory
					);
					numberOrder = numberOrder.filter(
						(r) => r.identifier !== newestRecord.identifier
					);
					...
					if (newestRecord.status === "active") {
						...
						if (numberOrder.length === 0) {
							...
							biggestNumber =
								numberOrder[numberOrder.length - 1].number;
						}
						...
						for (let i = 0; i < numberOrder.length; i++) {
							if (
								numberOrder[i].number < newestRecord.number &&
								(numberOrder[i + 1] === undefined ||
									newestRecord.number <=
										numberOrder[i + 1].number)
							) {
			...
			deleteRecord: async function (input) {
				...
				try {
					...
					result.success = true;
					result.message =
						"Record removed from community. Need to save community";
				} catch (error) {
					result.success = false;
					result.message = error;
				}
			...
			automaticVotes: async function (citizen) {
				...
				let result = {
					success: true,
					message: `automatic votes ran on ${citizen._id}`,
				};
				try {
			...
			addResident: async function (input) {
				...
				if (
					home.residents.some((resident) =>
						resident.equals(input.citizen._id)
					)
				) {
					result.success = false;
				...
				try {
					...
					result.success = true;
					result.message =
						"Citizen added as resident of home. Automatic votes ran.";

					let automaticVotesResult = this.automaticVotes(
						input.citizen
					);
					if (automaticVotesResult.success === false)
						result = automaticVotesResult;
				} catch (error) {
					result.success = false;
					result.message = error;
				}
			...
			removeResident: async function (input) {
				...
				if (input.home) {
					...
					if (
						!home.residents.includes(input.citizen._id.toString())
					) {
						result.success = false;
						result.message =
							"Citizen not registered as resident of given home";
						return result;
					}
					//remove citizen as resident from home
					home.residents = home.residents.filter(
						(resident) => !resident.equals(input.citizen._id)
					);
				} else {
			...
			addOwner: async function (input) {
				...
				try {
					...
					//automatic votes
					let automaticVotesResult = await this.automaticVotes(
						input.citizen
					);
					if (automaticVotesResult.success === false)
						result = automaticVotesResult;
			...
			createProposal: async function (input) {
				...
				if (!this.isCitizenMember(input.citizen._id)) {
					result.success = false;
					result.message =
						"This citizen is not a member of this community";
					return result;
				}
				//create new record
				//sanitize variables
				...
				newRecord.previousProposalLimit =
					input.proposal.previousProposalLimit;
			...
			vote: async function (input) {
				...
				if (
					input.vote.vote !== "plug" &&
					input.vote.vote !== "unplug"
				) {
					result.message =
						"input.citizen.vote.vote is not in a valid form";
					result.success = false;
					return result;
				}
				...
				let voteIndex = record.votes.findIndex((vote) =>
					vote.citizen.equals(input.citizen._id)
				);
				if (voteIndex === -1) {
					//this citizen hasn't voted on this record
		...
		virtuals: {
			citizens: {
				get() {
					...
					for (const home of this.homes) {
						if (home.owner) {
							let isInArray = citizens.some((citizen) =>
								citizen.equals(home.owner)
							);
							if (!isInArray) citizens.push(home.owner);
						}
						for (const resident of home.residents) {
							let isInArray = citizens.some((citizen) =>
								citizen.equals(resident)
							);
			...
			owners: {
				...
				get() {
					...
					for (const home of this.homes) {
						if (home.owner) {
							let isInArray = owners.some((owner) =>
								owner.equals(home.owner)
							);
			...
			laws: {
				get() {
					...
					let laws = this.records.filter(
						(record) =>
							record.status === "active" && record.type === "law"
					);
			...
			lawCategories: {
				get() {
					let categories = [];
					let activeLawsWithCategory = this.laws.filter(
						(record) => record.lawCategory
					);
					for (const record of activeLawsWithCategory) {
						if (!categories.includes(record.lawCategory))
							categories.push(record.lawCategory);
					}
					return categories;
				},
			},
			...
			proposals: {
				get() {
					...
					return this.records
						.filter((record) => record.status === "proposal")
		...
		statics: {
			async communitiesWhereResident(citizenId) {
				...
				const distinctCommunitiesId = await this.distinct("_id", {
					homes: { $elemMatch: { residents: citizenId } },
				});
			...
			async communitiesWhereOwner(citizenId) {
				...
				const distinctCommunitiesId = await this.distinct("_id", {
					homes: { $elemMatch: { owner: citizenId } },
				});
				return this.find({ _id: { $in: distinctCommunitiesId } });
			},
...
const citizenSchema = new Schema(
	{
		...
		cellPhoneConfirm: { type: Boolean, default: false },
...
const communityRequest = mongoose.model(
	"communityRequest",
...
async function createCommunity(communityRequest) {
	...
	let homes = [];
	for (
		let i = communityRequest.homesStartingNumber;
		i <= communityRequest.homesEndingNumber;
		i++
	) {
		homes.push({ number: i });
	}
